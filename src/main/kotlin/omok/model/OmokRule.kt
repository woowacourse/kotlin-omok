...
class OmokRule(
    private val board: Array<Array<Stone>>,
    private val currentStone: Stone = Stone.BLACK_STONE,
    private val otherStone: Stone = Stone.WHITE_STONE,
    ...
    fun checkRule(
        ...
        return checkThreeThree(x, y) || checkFourFour(x, y) || checkMoreThanFive(x, y)
    }
    ...
    fun validateWhiteWin(
        x: Int,
        y: Int,
    ): Boolean = directions.map { direction -> checkWhiteWin(x, y, direction[0], direction[1]) }.contains(true)

    fun validateBlackWin(
        x: Int,
        y: Int,
    ): Boolean = directions.map { direction -> checkBlackWin(x, y, direction[0], direction[1]) }.contains(true)
    ...
    companion object {
        private val EMPTY_STONE = Stone.NONE
