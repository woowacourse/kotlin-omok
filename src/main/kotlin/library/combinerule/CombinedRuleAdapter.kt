...
class CombinedRuleAdapter : GameRule {
    override fun isBlackWin(position: Position, board: BoardState): Boolean =
        BlackWinRule.validate(toCombineRuleBoard(board.value), position.coordinateX to position.coordinateY)

    override fun isWhiteWin(position: Position, board: BoardState): Boolean =
        WhiteWinRule.validate(toCombineRuleBoard(board.value), position.coordinateX to position.coordinateY)
