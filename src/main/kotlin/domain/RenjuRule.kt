...
object RenjuRule {
    fun isThreeToThree(stones: Stones, stone: Stone): Boolean {
        return directions.sumOf { direction ->
            checkOpenFourForLine(stones, (direction * -4) + stone.coordinate.point, direction)
        } >= RENJU_LINE_CONDITION
    }

    private fun checkOpenFourForLine(stones: Stones, start: Point, direction: Point): Int {
        return OPEN_FOUR_INNER_BLOCK_RANGE.count {
            isOpenFour(stones, start + (direction * it), direction)
        }
    }

    private fun isOpenFour(stones: Stones, start: Point, direction: Point): Boolean {
        ...
        val isFour = OPEN_FOUR_INNER_BLOCK_SHIFTED_RANGE.count {
            isBlackStoneThere(stones, start + (direction * it))
        } == OPEN_FOUR_BLACK_STONE_COUNT
        return isOpened && isFour
    }

    fun isFourToFour(stones: Stones, stone: Stone): Boolean {
        return fullDirections.count { direction ->
            checkFourForLine(stones, (direction * -4) + stone.coordinate.point, direction)
        } >= RENJU_LINE_CONDITION
    }

    private fun checkFourForLine(stones: Stones, start: Point, direction: Point): Boolean {
        return FOUR_BLOCK_RANGE.any {
            isFour(stones, start + (direction * it), direction)
        }
    }

    private fun isFour(stones: Stones, start: Point, direction: Point): Boolean {
    ...
    private fun isBlackStoneThere(stones: Stones, start: Point): Boolean {
    ...
    fun findScore(stones: Stones, stone: Stone): Int {
        return directions.maxOf { item ->
            val nextCoordinate = (stone.coordinate + item) ?: return@maxOf 0
            val score = startSearch(stones, nextCoordinate, item, stone.color, INITIAL_SCORE)
            val invertedDirections = item * -1
            val invertCoordinate = (stone.coordinate + invertedDirections) ?: return@maxOf 0
            val invertedScore = startSearch(stones, invertCoordinate, invertedDirections, stone.color, INITIAL_SCORE)
            score + invertedScore
        }
    }

    private fun startSearch(stones: Stones, coordinate: Coordinate, direction: Point, color: Color, count: Int): Int {
        if (stones.value.any { it.coordinate == coordinate && it.color == color }) {
            ...
            return startSearch(stones, nextCoordinate, direction, color, count + SEARCH_INTERVAL)
        }
        return count
    }
