...
import domain.rule.type.Violation.Companion.FOUL_CONDITION_SIZE
import domain.rule.type.Violation.Companion.MAX_EMPTY_SIZE
import domain.rule.type.Violation.Companion.OVERLINE_SIZE
import domain.rule.type.WhiteBlocked
...
class BlackRenjuRule(
    ...
    override fun checkDoubleFoul(
        ...
        startPoint: Point,
        foul: Foul,
    ): Violation = checkFoulByAllDirections(myPoints, otherPoints, startPoint, foul)
    ...
    private fun checkFoulByAllDirections(
        ...
        while (dirIterator.hasNext()) {
            ...
            val (forwardCount, forwardEmptyCount) = findStraight(
                myPoints, otherPoints, startPoint,
                forwardDir, foul,
            )
            ...
            when (foul) {
                Foul.DOUBLE_THREE -> {
                    if (totalStoneCount == foul.size && totalEmptyCount <= MAX_EMPTY_SIZE) {
                        val blockedStatus = isBlockedByWhiteStoneInSix(otherPoints, startPoint, forwardDir)
