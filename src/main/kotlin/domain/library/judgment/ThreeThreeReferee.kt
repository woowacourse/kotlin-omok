...
class ThreeThreeReferee : PlacementReferee() {
    ...
    private fun countThree(board: Map<Position, Color?>, position: Position): Int {
        return listOf(
    ...
    private fun isVerticalThree(board: Map<Position, Color?>, position: Position): Boolean {
    ...
    private fun isUpwardDiagonalThree(board: Map<Position, Color?>, position: Position): Boolean {
        val northEastEmptyPosition = findNorthEastEmptyPosition(board, position)
        if (northEastEmptyPosition != null && isUpwardDiagonalOpenFour(
                board.toMutableMap(),
                northEastEmptyPosition
            )
        ) return true
        val southWestEmptyPosition = findSouthWestEmptyPosition(board, position)
        if (southWestEmptyPosition != null && isUpwardDiagonalOpenFour(
                board.toMutableMap(),
                southWestEmptyPosition
            )
        ) return true
        return false
    }
    ...
    private fun isUpwardDiagonalOpenFour(board: MutableMap<Position, Color?>, position: Position): Boolean {
        ...
        if (countUpwardDiagonalContinuity(board, position, 0) == 4) return isNorthEastOpen(
            board,
            position
        ) && isSouthWestOpen(board, position)
        return false
    }
    ...
    private fun isHorizontalThree(board: Map<Position, Color?>, position: Position): Boolean {
    ...
    private fun isDownwardDiagonalThree(board: Map<Position, Color?>, position: Position): Boolean {
        val southEastEmptyPosition = findSouthEastEmptyPosition(board, position)
        if (southEastEmptyPosition != null && isDownwardDiagonalOpenFour(
                board.toMutableMap(),
                southEastEmptyPosition
            )
        ) return true
        val northWestEmptyPosition = findNorthWestEmptyPosition(board, position)
        if (northWestEmptyPosition != null && isDownwardDiagonalOpenFour(
                board.toMutableMap(),
                northWestEmptyPosition
            )
        ) return true
        return false
    }
    ...
    private fun isDownwardDiagonalOpenFour(board: MutableMap<Position, Color?>, position: Position): Boolean {
        ...
        if (countDownwardDiagonalContinuity(board, position, 0) == 4) return isSouthEastOpen(
            board,
            position
        ) && isNorthWestOpen(board, position)
        return false
    }
