...
data class BlackPlayer(
                ...
                when (calculateType) {
                    ...
                    CalculateType.ThreeToThreeCount ->
                        CalculateType.ThreeToThreeCount.checkCalculateType {
                            ++threeToThreeCount >= Player.MIN_THREE_TO_THREE_COUNT
                        }
                }
            }
    ...
    private inline fun getCalculateType(
        ...
        if (!result.isFirstClear) {
            ...
            } else if (result.count == EDGE_THREE_TO_THREE_COUNT && reverseResultCount == EDGE_FOUR_TO_FOUR_COUNT && !isReverseResultFirstClear) {
                type(
                    CalculateType.IsReverseTwoAndThree,
                )
            }
