...
class TurnTest {
    ...
    fun `오목판에 흑이 돌을 둔 다음 백이 두어야 한다`() {
        ...
        val actual = (whiteTurn.placeStone(Point(1, 1)) as Either.Right).value
    ...
    fun `오목판에 백이 돌을 둔 다음 흑이 두어야 한다`() {
        ...
        val actual = (blackTurn.placeStone(Point(9, 9)) as Either.Right).value
    ...
    fun `이전 턴에 어떤 곳에 돌을 놓았는지 알 수 있다`() {
        val board = Board()
        val point = Point(1, 1)
        val boardAfterPlace = ((WhiteTurn(board).placeStone(point)) as Either.Right).value.board
