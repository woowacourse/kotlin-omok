package omok.domain

import omok.POINT_H6
import omok.beforeDoubleFour
import omok.beforeDoubleThree
import omok.beforeOverLine
import omok.domain.point.Column
import omok.domain.point.OmokPoint
import omok.domain.point.Row
import omok.domain.rule.BlackRuleAdapterImpl
import omok.domain.rule.Referee
import omok.domain.rule.WhiteRuleAdapterImpl
import omok.getFoulPoint
import omok.omokPoints
import org.assertj.core.api.Assertions.assertThat
import org.junit.jupiter.api.BeforeEach
import org.junit.jupiter.api.Test
import org.junit.jupiter.api.assertDoesNotThrow
import org.junit.jupiter.api.assertThrows

class RefereeTest {
    private val referee = Referee()
    private lateinit var grid: OmokGrid

    @BeforeEach
    fun setUp() {
        grid = OmokGrid()
    }

    @Test
    fun `흑돌은 3x3 위치에 돌을 놓을 수 없다`() {
        // given
        val beforeDoubleThree = beforeDoubleThree()
        beforeDoubleThree.forEach {
            grid.blackStones + it
        }
        // when & then
        assertThrows<IllegalStateException> {
            referee.checkViolation(BlackRuleAdapterImpl, grid, getFoulPoint())
        }
    }

    @Test
    fun `백돌은 3x3 위치에 돌을 놓을 수 있다`() {
        // given
        val beforeDoubleThree = beforeDoubleThree()
        beforeDoubleThree.forEach {
            grid.whiteStones + it
        }
        // when & then
        assertDoesNotThrow {
            referee.checkViolation(WhiteRuleAdapterImpl, grid, getFoulPoint())
        }
    }

    @Test
    fun `흑돌은 4x4 위치에 돌을 놓을 수 없다`() {
        // given
        val beforeDoubleFour = beforeDoubleFour()
        beforeDoubleFour.forEach {
            grid.blackStones + it
        }
        // when & then
        assertThrows<IllegalStateException> {
            referee.checkViolation(BlackRuleAdapterImpl, grid, getFoulPoint())
        }
    }

    @Test
    fun `백돌은 4x4 위치에 돌을 놓을 수 있다`() {
        // given
        val beforeDoubleFour = beforeDoubleFour()
        beforeDoubleFour.forEach {
            grid.whiteStones + it
        }
        // when & then
        assertDoesNotThrow {
            referee.checkViolation(WhiteRuleAdapterImpl, grid, getFoulPoint())
        }
    }

    @Test
    fun `흑돌은 장목 위치에 돌을 놓을 수 없다`() {
        // given
        val beforeOverLine = beforeOverLine()
        beforeOverLine.forEach {
            grid.blackStones + it
        }
        // when & then
        assertThrows<IllegalStateException> {
            referee.checkViolation(BlackRuleAdapterImpl, grid, getFoulPoint())
        }
    }

    @Test
    fun `백돌은 장목 위치에 돌을 놓을 수 있다`() {
        // given
        val beforeOverLine = beforeOverLine()
        beforeOverLine.forEach {
            grid.whiteStones + it
        }
        // when & then
        assertDoesNotThrow {
            referee.checkViolation(WhiteRuleAdapterImpl, grid, getFoulPoint())
        }
    }

    @Test
    fun `이미 돌이 있는 위치에 돌을 놓을 수 없다`() {
        // given
        grid.blackStones + OmokPoint(Row(1), Column(2))
        // when & then
        assertThrows<IllegalStateException> {
            referee.checkViolation(BlackRuleAdapterImpl, grid, OmokPoint(Row(1), Column(2)))
        }
        assertThrows<IllegalStateException> {
            referee.checkViolation(WhiteRuleAdapterImpl, grid, OmokPoint(Row(1), Column(2)))
        }
    }

    @Test
    fun `돌 다섯개가 연속으로 이어지면 오목이다`() {
        // given
        val omokStones = omokPoints()
        omokStones.forEach {
            grid.blackStones + it
        }
        // when
        val actual = referee.checkWin(BlackRuleAdapterImpl, grid.blackStones.stones, POINT_H6)
        // then
        assertThat(actual).isTrue()
    }
}
