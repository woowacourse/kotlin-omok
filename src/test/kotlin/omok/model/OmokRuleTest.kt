...
class OmokRuleTest {
    ...
    fun `checkThreeThree를 확인한다 - 첫번째 경우`() {
        listOf(C12, E12, D13, D14).map {
            board.layout[it.coordinate.x][it.coordinate.y] = StoneType.BLACK_STONE
        }
    ...
    fun `백돌이 오목인지 확인한다`() {
        listOf(A1, A2, A3, A4, A5).map {
            board.layout[it.coordinate.x][it.coordinate.y] = StoneType.WHITE_STONE
        }
        BoardView.printBoard(board)
        assertThat(
            OmokRule(board.layout, currentStone = StoneType.WHITE_STONE, otherStone = StoneType.BLACK_STONE).validateOmok(0, 4),
        ).isEqualTo(true)
    }
