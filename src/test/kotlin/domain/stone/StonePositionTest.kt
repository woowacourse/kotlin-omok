...
class StonePositionTest {
    ...
    @Test
    fun `x좌표와 y좌표를 알고 있다`() {
        val stonePosition: StonePosition = StonePosition(x = 5, y = 7)
        assertAll(
    ...
    fun `x좌표는 1이상 15이하의 값을 가진다`(x: Int) {
        val stonePosition: StonePosition = StonePosition(x, y = 5)
    ...
    fun `x좌표가 1미만 15초과인 경우 예외를 발생시킨다`(x: Int) {
        assertThrows<IllegalArgumentException> { StonePosition(x, y = 5) }
    }

    @ParameterizedTest
    @ValueSource(ints = [1, 15])
    fun `y좌표는 1이상 15이하의 값을 가진다`(y: Int) {
        val stonePosition: StonePosition = StonePosition(x = 5, y)
    ...
    fun `y좌표가 1미만 15초과인 경우 null을 반환한다`(y: Int) {
        assertThrows<IllegalArgumentException> { StonePosition(x = 5, y) }
    }
}
