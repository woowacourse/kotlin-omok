...
class StonesTest {
    ...
    @Test
    fun `스톤들의 정보를 담고 있다`() {
        val stonePosition = StonePosition(1, 1)
    ...
    fun `스톤을 받아 해당 스톤의 위치에 돌이 놓여있는지 확인`() {
        val stones = Stones()
        stones.add(Stone(StonePosition(1, 1), StoneType.BLACK))

        val stone = Stone(StonePosition(1, 1), StoneType.BLACK)
    ...
    fun `스톤들을 받아서 두 스톤들을 더한 값을 반환한다`() {
        ...
        blackStones.add(Stone(StonePosition(1, 1), StoneType.BLACK))
        whiteStones.add(Stone(StonePosition(2, 2), StoneType.WHITE))
