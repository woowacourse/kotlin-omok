...
class PlayingStateTest {
    ...
    fun `오목알을 놓으면 목록에 추가한다`() {
        val playingState = PlayingState(Points(ONE_ONE_STONE, ONE_TWO_STONE)).add(ONE_THREE_STONE)
    ...
    fun `돌을 놓았을 때 오목알 연이어져 있는 오목알이 5개 미만이면 게임을 계속한다`() {
        val playingState = PlayingState(Points(ONE_ONE_STONE, ONE_TWO_STONE))
    ...
    fun `돌을 놓았을 때 오목알 5개 이상이 연이어져 있으면 승리한다`() {
        val playingState = PlayingState(Points(ONE_ONE_STONE, ONE_TWO_STONE, ONE_THREE_STONE, ONE_FOUR_STONE))
