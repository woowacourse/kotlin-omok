...
class PlayersTest {
    ...
    fun `첫 턴은 흑이며, 현재 턴의 플레이어가 오목알을 성공적으로 놓으면 턴이 백으로 바뀐다`() {
        val players = Players.from(BlackPlayer(rule = blackRenjuRule), WhitePlayer(rule = whiteRenjuRule))
        val actual = players.putPoint(Point(1, 1))
        val expected = Players.from(
    ...
    fun `현재 턴의 플레이어의 돌 색깔을 반환한다`() {
        val players =
            Players.from(BlackPlayer(rule = blackRenjuRule), WhitePlayer(rule = whiteRenjuRule))
