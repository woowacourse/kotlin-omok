...
class PlacedBoardTest {
    ...
    fun `이미 놓인 위치에 돌을 놓으면 자기 자신을 반환한다`() {
        val placedBoard = PlacedBoard().putStone(Position(1, 3), Color.BLACK)
        val nextBoard = placedBoard.putStone(Position(1, 3), Color.WHITE)
    ...
    fun `돌이 없는 곳에 돌을 놓을 수 있다`() {
        val placedBoard = PlacedBoard().putStone(Position(1, 3), Color.BLACK)
        val nextBoard = placedBoard.putStone(Position(2, 3), Color.WHITE)
        val nextBoardState =
