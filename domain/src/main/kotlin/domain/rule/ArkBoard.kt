...
class ArkBoard(
    ...
    fun placeStone(point: OmokPoint, stoneState: StoneState = this.stoneState): ArkBoard {
        ...
        newValue[point.y] =
            newValue[point.y]?.placeStone(point, stoneState) ?: throw IllegalArgumentException()
        return ArkBoard(newValue, stoneState.next())
    }

    operator fun get(yCoordinate: YCoordinate): OmokLine =
        value[yCoordinate] ?: throw IllegalArgumentException()
