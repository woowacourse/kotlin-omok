...
class GameTest {
    ...
    fun `오목이 완성되면 게임이 종료된다 1`() {
        val game =
            Game(Board(), RenjuRule()).apply {
                ...
                play(Stone(Position(Col(1), Row(4)), Color.WHITE))
            }

        val actual: MoveResult = game.play(Stone(Position(Col(1), Row(5)), Color.WHITE))
    ...
    fun `오목이 완성되면 게임이 종료된다 2`() {
        val game =
            Game(Board(), RenjuRule()).apply {
                ...
                play(Stone(Position(Col(10), Row(8)), Color.BLACK))
            }

        val actual: MoveResult = game.play(Stone(Position(Col(9), Row(9)), Color.BLACK))
        val expectedClass = MoveResult.Success.Finished::class.java
    ...
    fun `오목이 완성되지 않았으면 게임이 진행 중이다`() {
        ...
        val actual: MoveResult = game.play(Stone(Position(Col(9), Row(9)), Color.WHITE))
