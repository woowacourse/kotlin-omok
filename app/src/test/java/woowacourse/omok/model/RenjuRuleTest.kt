...
class RenjuRuleTest {
    @Test
    fun `이미 돌이 있는 위치에는 돌을 둘 수 없다`() {
        val board = Board().apply { add(Stone(Position(Col(1), Row(1)), Color.BLACK)) }

        val actual: MoveResult = RuleAdapter(BlackRenjuRule()).checkMove(board, Stone(Position(Col(1), Row(1)), Color.BLACK))
    ...
    fun `흑은 삼삼 위치에 돌을 둘 수 없다`() {
        ...
        val actual: MoveResult = RuleAdapter(BlackRenjuRule()).checkMove(board, Stone(Position(Col(7), Row(4)), Color.BLACK))
    ...
    fun `흑은 사사 위치에 돌을 둘 수 없다`() {
        ...
        val actual: MoveResult = RuleAdapter(BlackRenjuRule()).checkMove(board, Stone(Position(Col(12), Row(6)), Color.BLACK))
    ...
    fun `흑은 장목을 둘 수 없다`() {
        ...
        val actual: MoveResult = RuleAdapter(BlackRenjuRule()).checkMove(board, Stone(Position(Col(13), Row(3)), Color.BLACK))
