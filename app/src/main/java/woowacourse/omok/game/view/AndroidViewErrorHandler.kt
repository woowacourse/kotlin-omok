...
class AndroidViewErrorHandler(val context: Context) : ErrorHandler {
    override fun log(exception: OmokError) {
        Toast.makeText(
            ...
            "[ERROR] : ${
            when (exception) {
                is CoordinateError.OutOfBoard -> MESSAGE_OUT_OF_BOARD

                is PlaceStoneError.LongPlaceStone -> MESSAGE_LONG_STONE

                is PlaceStoneError.FourToFour -> MESSAGE_FOUR_FOUR

                is PlaceStoneError.ThreeToThree -> MESSAGE_THREE_THREE

                is PlaceStoneError.DuplicatedCoordinate -> MESSAGE_CORRUPTED_COORDINATE

                is StoneReadError.ColumnNotAlpha -> MESSAGE_COLUMN_MUST_BE_ALPHA

                is StoneReadError.RowNotNumeric -> MESSAGE_ROW_MUST_BE_NUM
